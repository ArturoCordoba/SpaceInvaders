...
public class LinkedList<T extends Comparable<T>> {
     ...
     */
    public Node getElement(Integer position){ //El primer elemento de la list es el elemento número 0
     ...
     */
    public void insertElement(T data, Integer position){
        Node newNode = new Node(data);
        Node current = this.first;

        if (this.first != null) {
            if (1 <= position && position < this.size) {
                for (Integer i = 0; i < (position - 1); i++) { //Se detiene en el nodo anterior al de la posición deseada
                    current = current.getNext();
                }
            ...
            if (this.size <= position) {
                for (Integer i = 0; i < (this.size - 1); i++) { //Se detiene en el último nodo de la lista
                    current = current.getNext();
                }
                current.setNext(newNode);
            }
     ...
     */
    public void deleteElement(Integer position) {
        if (position == 0) {
            this.deleteFirst();
        } else {
    ...
    /**
     * Metodo para verificar si dentro de la lista existe ya un elemento
     * @param element Elemento a verificar
     * @return True si ya esta dentro de la lista, false en caso contrario
     */
    public boolean contains(T element){
        ...
        while(current != null){
            if(current.getDataT().compareTo(element) == 0){
                contains = true;
                break;
            }
            current = current.getNext();
